...
const domain = ref('');
const record = ref('');
const tld = ref('programwithwebmoss');
...
async function mintDomain() {
  ...
  const price = domain.value.length === 3 ? '0.05' : domain.value.length === 4 ? '0.03' : '0.01';
...
<template>
  <div className="form-container">
    ...
    <div className="button-container">
      <button className="cta-button mint-button" @click="mintDomain">Mint</button>
    </div>
  </div>
</template>
